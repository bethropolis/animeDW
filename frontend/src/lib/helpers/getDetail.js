...
export async function getDetail(url) {
  ...
  const author = $('div:contains("Studios:") span  [itemprop="name"]').text().trim();
  if (author) {
    anime.author = author;
  }
  ...
  const title = $('.info h1.title').text().trim();
  if (title) {
    anime.title = title;
  }
  ...
  const resEp = await Client.get(`/ajax/episode/list/${id}?${vrf}`);
